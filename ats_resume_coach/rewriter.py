...
def _build_profile(parsed: ParsedResume, analysis: AnalysisResult, job_text: str, resume_text: str) -> str:
    ...
    experience = _experience_profile_fragment(parsed)
    ...
    sentences.append(" ".join(opener_parts).strip() + ".")

    if strengths:
        sentences.append("Experienced in " + _join_human(strengths[:5]) + ".")
    if leadership:
...
def _education_profile_fragment(parsed: ParsedResume) -> str | None:
    ...
    if not education:
        return None
    text = " ".join(line.text for line in education.lines)
    degree_match = re.search(
        r"\b(?:MEng|BSc|BA|MSc|BS|MS|PhD|Bachelor(?:'s)?|Master(?:'s)?)[^,.;|\n]{0,90}",
        text,
        re.IGNORECASE,
    )
    institution = None
    for line in education.lines:
        if "university" in line.text.lower() or "college" in line.text.lower():
            if "—" in line.text or " - " in line.text:
                institution = re.split(r"\s+[—|-]\s+", line.text, maxsplit=1)[0].strip()
            elif "," in line.text:
                institution = line.text.rsplit(",", 1)[-1].strip()
            else:
                institution = re.split(r"\b(?:MEng|BSc|BA|MSc|BS|MS|PhD|Bachelor|Master)\b", line.text, maxsplit=1)[0].strip()
            break
    if degree_match and institution:
        degree = degree_match.group(0).strip()
        return f"{degree} student at {institution}"
    if degree_match:
        return f"{degree_match.group(0).strip()} student"
    return None


def _experience_profile_fragment(parsed: ParsedResume) -> str | None:
...
def _polished_experience_lines(section: SourceSection, styles: ResumeStyles, analysis: AnalysisResult) -> list[SourceLine]:
    ...
    for index, line in enumerate(section.lines):
        ...
        elif _is_role_or_date_line(line):
...
def _polish_bullet(text: str, analysis: AnalysisResult | None) -> str:
    ...
    if not ACTION_START_RE.search(cleaned) and cleaned:
...
def _replace_weak_opening(text: str) -> str:
    ...
    if lowered.startswith("core ") and " developer on " in lowered:
        return re.sub(r"^core .*? developer on ", "Develop ", text, flags=re.IGNORECASE)
    return text
...
def _add_truthful_context(text: str, analysis: AnalysisResult) -> str:
    ...
    for values in analysis.matched_skills.values():
        truthful_terms.extend(values)
    for term in _dedupe(truthful_terms)[:3]:
        if term.lower() not in lowered and any(signal in lowered for signal in ("using", "with", "across", "for")):
            continue
    return text
